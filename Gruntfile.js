...
module.exports = function (grunt) {
    ...
    grunt.initConfig({
        ...
        // local variables
        vars : {
            timestamp : '<%= grunt.template.today("yyyymmddHHMMss") %>', appName : pkgJson.name, appVer : pkgJson.version, baseFilename : pkgJson.appFilename
        },

        // DRY-ing directories through local variables
        dirs : {
            srcClient : 'client', srcClientJs : 'client/src/js', srcClientStyles : 'client/src/styles', srcClientAssets : 'client/assets', srcClientTests : 'client/test', srcServer : 'server', srcServerJs : 'server/src', srcServerTests : 'server/test', appBuild : 'build/app', publicBuild : 'build/public', target : 'server/public', uiTests : 'test', uiTestsCasper : 'test/casperjs'
        },

        // JsHint options :: see http://www.jshint.com/options/
        jshint : {
            grunt : {
                options : jsHintOptions.code, files : { src : ['Gruntfile.js', '.jshintoptions.js'] }
            }, client : {
                options : jsHintOptions.code, files : { src : ['<%= dirs.srcClientJs %>/**/*.js'] }
            }, server : {
                options : jsHintOptions.code, files : { src : ['<%= dirs.srcServer %>/server.js', '<%= dirs.srcServerJs %>/**/*.js'] }
            }, testClient : {
                options : jsHintOptions.tests, files : { src : ['<%= dirs.srcClientTests %>/**/*.js'] }
            }, testServer : {
                options : jsHintOptions.tests, files : { src : ['<%= dirs.srcServerTests %>/**/*.js'] }
            }, appUiTests : {
                options : jsHintOptions.code, files : { src : ['<%= dirs.uiTestsCasper %>/**/*.js'] }
            }
        },

        // cleaning target directory
        clean : {
            build : ['build/*'], public : ['server/public']
        },
        ...
        requirejs : {
            compile : {
                options : {
                    mainConfigFile : '<%= dirs.srcClientJs %>/config.js'                     /* Include the main configuration file */, out : '<%= dirs.appBuild %>/js/<%= vars.baseFilename %>.js'   /* Output file */, name : 'config'                                                /* Root application module */, wrap : false                                                   /* Do not wrap everything in an IIFE */, optimize : 'none'                                                  /* JS-optimization on build: 'none' or 'uglify' supported via Node */, insertRequire : ['main'], logLevel : 1
                }
            }
        },
        ...
        uglify : {
            ...
            release : {
                src : '<%= dirs.appBuild %>/js/<%= vars.baseFilename %>.js', dest : '<%= dirs.appBuild %>/js/<%= vars.baseFilename %>-<%= vars.appVer %>.min.js'
            }
        },
        ...
        replace : {
            ...
            dev : {
                options : {
                    variables : {
                        appScriptTag : '<script data-main="/src/js/config" src="http://cdnjs.cloudflare.com/ajax/libs/require.js/2.1.4/require.min.js"></script>', appStyleTag : '<link rel="stylesheet/less" type="text/css" href="/src/styles/style.less" media="all">', lessScriptTag : '<script src="http://cdnjs.cloudflare.com/ajax/libs/less.js/1.3.3/less.min.js" type="text/javascript"></script>'
                    },
                    force : true
                },
                files : [
            ...
            debug : {
                options : {
                    variables : {
                        appScriptTag : '<script src="/assets/js/<%= vars.baseFilename %>.js?<%= vars.timestamp %>"></script>', appStyleTag : '<link rel="stylesheet" type="text/css" href="/<%= vars.baseFilename %>.css?<%= vars.timestamp %>"/>', lessScriptTag : ''
                    },
                    force : true
                },
                files : [
            ...
            release : {
                options : {
                    variables : {
                        appScriptTag : '<script src="/assets/js/<%= vars.baseFilename %>-<%= vars.appVer %>.min.js"></script>', appStyleTag : '<link rel="stylesheet" type="text/css" href="/<%= vars.baseFilename %>-<%= vars.appVer %>.min.css"/>', lessScriptTag : ''
                    },
                    force : true
                },
                files : [
        ...
        simplemocha : {
            options : {
                timeout : 3000, ignoreLeaks : false, globals : ['NODE_CONFIG'], ui : 'bdd', reporter : 'dot'
            },
            server : {
